...
class LeakDetector:
    ...
    def analyze_snapshots(self, snapshots: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        leak_detected = (
            is_trending_up and 
            growth_rate > self.sensitivity and
            coefficient_of_variation < 0.3  # Consistent growth
        )
        ...
        if leak_detected:
            if growth_rate * 60 > 10:  # >10MB/min
                ...
                result['recommendation'] = 'Critical: Investigate immediately. Memory growing rapidly.'
            elif growth_rate * 60 > 1:  # >1MB/min
                result['severity'] = 'medium'
                result['recommendation'] = 'Warning: Potential memory leak detected. Monitor closely.'
            else:
                result['severity'] = 'low'
                result['recommendation'] = 'Info: Slow memory growth detected. May be normal for your workload.'
        else:
    ...
    def _is_trending_upward(self, values: List[float], window: int = 5) -> bool:
        ...
        return increases / (len(recent_values) - 1) > 0.6 if len(recent_values) > 1 else False
    
    def detect_anomaly(self, snapshots: List[Dict[str, Any]], 
